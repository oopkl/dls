...
vector<string> depthLimitedSearch(const string &start, const string &goal, int limit) {
    vector<string> path;
    map<string, bool> visited;
    if (depthLimitedSearch(start, goal, limit, path, visited)) {
...
int main() {
    string start = "A";
    string goal = "G";
    int limit = 4; // Set a depth limit

    vector<string> path = depthLimitedSearch(start, goal, limit);

    if (!path.empty()) {
        cout << "Path from " << start << " to " << goal << " with depth limit " << limit << ":\n";
        for (const string &city : path) {
            cout << city << " -> ";
        }
        cout << "Goal reached.\n";
    } else {
        cout << "No path found from " << start << " to " << goal << " within depth limit " << limit << ".\n";
    }
